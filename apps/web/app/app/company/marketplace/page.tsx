...
export default function CompanyMarketplacePage() {
  ...
  async function buyPack(id: string) {
    ...
    if (result.status === 'success') {
      setMessage('Pack purchased and certificate retired successfully.');
      return;
    }
  ...
  return (
    <div>
      ...
      <main className="container mx-auto py-16">
        ...
        <p className="mt-2 text-slate-600">Buy retired impact certificates for transparent reporting.</p>
        ...
        <div className="mt-8 grid gap-6 md:grid-cols-2">
          {packs.map(pack => (
            <div key={pack.id} className="rounded-3xl bg-white p-6 shadow-sm">
              ...
              <p className="mt-4 text-slate-700">€{(pack.priceCents / 100).toFixed(0)} · {pack.treesPlanted} trees</p>
              <button onClick={() => buyPack(pack.id)} className="mt-6 rounded bg-slate-900 px-4 py-2 text-white">Buy pack</button>
            </div>
          ))}
        </div>
      </main>
